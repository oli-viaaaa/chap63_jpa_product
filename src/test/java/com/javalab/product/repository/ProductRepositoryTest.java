...
public class ProductRepositoryTest {
	...
	/*
	 * Product 더미 데이터 저장 - Validation 조건을 체크하고 그에 맞게 샘플 데이터를 입력할것. 조건에 맞지 않으면 오류나며
	 * 원인을 찾기 힘듦.
	 */
	//@Test
	// @Disabled
...
//		log.info("Product 저장 완료!");	
	    ...
	    IntStream.rangeClosed(101,200).forEach(i -> {
	    	Product product = Product.builder().productName("product Builder" + i).description("product Builder 설명" + i).build();
	...
	/*
	 * Product 목록 조회(페이지네이션/정렬 기능 구현) Spring Data Jpa의 페이징, 정렬은 findAll(pageable)메소드
	 * 사용 - findAll(Pageable) : PagingAndSortRepository 소속 메소드 - Page<T> : 반환타입이
	 * Page<T> 타입인 경우 에는 인자가 반드시 Pageable이어야 함. - Pageable : 페이징 관련 정보를 담고 있는 객체가
	 * 구현해야 할 인터페이스. 어떻게 페이징과 정렬을 해야 하는지에 대한 정보가 있다. - PageRequest : Pageable 인터페이스를
	 * 구현한 구현체(요청페이지, 게시물수, 정렬) - static of() : PageRequest 객체를 얻기 위해서 필요한 정적 메소드 -
	 * Sort.by("id") : 정렬 컬럼(필드)
	 */
	// @Test
	// @Disabled
	 ...
	 * 
	 * [Fetch 전략 - Eager / Lazy]
	 * 	- Fetch 전략을 Lazy로 하게 되면 지연로딩이 되어 처음에는 상품 테이블만 조회하고 카테고리 테이블이 필요한 시점 즉, prduct.toString()이 되는 시점에
	 * 	  카테고리 테이블에 조회해서 카테고리 관련 모든 정보를 조회한다.
	 * 	- Fetch 전략을 안주거나, Eager로 하게 되면 처음 부터 상품 테이블과 카테고리 테이블을 조인걸게 된다.
	 * 	  그렇게 되면 데이터베이스에 성능에 좋지 않다. 가급적이면 지연로딩을 사용하는게 낫다.
	 * 	- @ManyToOne : 기본이 즉시로딩(Eager)
	 * 	- @OneRoMay : 기본이 지연로딩(Lazy)
	 */
	// @Test
	// @Disabled
	public void testGetProductById1() {
		...
		// 1. 상품 엔티티의 Category 멤버 변수에 지연로딩을 설정해놓게 되면 이 시점까지는 상품 테이블만 조회함.
		// 2. 아래의 product.toString()으로 상품 정보의 모든 내용(Category 포함)을 보겠다고 하는 시점에 그제서야 카테고리
		// 테이블에 조회를 한다.
	...
	public void testGetProductById2() {
		Optional<Product> optProduct = productRepository.findById(30); // 데이터베이스 확인
		Product product = optProduct.get();
	...
	public void testProductUpdate() {
		...
		Product product = new Product();
		product.setProductId(81); // 실존하는 상품명으로
